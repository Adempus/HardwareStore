 ...
 * i accesses a vector for every n mod 2 != 0, denoting an item's purchase quantity to pair second.
 *                   S, 3, grill, 1, hammer, 3,  wrench, 5
                i:               |3|        |5|         |7
 */
void shopperEvent(string& data) {
    ...
    for (int i = 0, j = 0 ; getline(ss, parsedDatum, ',') ; j++) {
        ...
        if (parsedDatum != "S" && j % 2 == 0) {
            ...
            // use i (when j % 2 != 0) to access the vector and pair the Item with its int quantity
